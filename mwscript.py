...
def run(args: argparse.Namespace) -> None:
    ...
    if len(script.split('/')) == 1:
        script = f'/var/www/html/mediawiki1/maintenance/{script}'
    else:
        scriptsplit = script.split('/')
        script = f'/var/www/html/mediawiki1/{scriptsplit[0]}/{scriptsplit[1]}/maintenance/{scriptsplit[2]}'
    ...
    if wiki == 'all':
        long = True
        command = f'sudo -u www-data bash /var/www/html/mediawiki1/foreach /var/www/html/mediawiki1/cache/databases.json {script}'
    elif wiki in validDBLists:
        long = True
        command = f'sudo -u www-data bash /var/www/html/mediawiki1/foreach /var/www/html/mediawiki1/cache/{wiki}.json {script}'
    elif args.extension:
        long = True
        ...
        command = f'sudo -u www-data /var/www/html/mediawiki1/foreachwikiindblist /home/{os.getlogin()}/{args.extension}.json {script}'
    else:
        command = f'sudo -u www-data php {script} --wiki={wiki}'
    if args.arguments:
        command += ' ' + ' '.join(args.arguments)
    logcommand = f'/var/www/html/mediawiki1/log "{command}'
    print('Will execute:')
